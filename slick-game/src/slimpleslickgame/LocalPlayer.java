...
public class LocalPlayer extends Player{
	...
	private boolean processInput(Input input) {
		...
		if(input.isKeyPressed(Input.KEY_SPACE)) {
			...
			byte[] bytes = MessageWrapper.appendByteArray(new byte[]{EventProtocol.BULLET_INIT, EventProtocol.BULLET_ID, (byte) this.gun.getbulletID(), EventProtocol.BULLET_POS}, MessageWrapper.getVector2fBytes(shotPos));
			bm.putArrayToServer(bytes, super.id);
		}
